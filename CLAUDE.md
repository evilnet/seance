...
- **A message may span several lines.** `draft/multiline` (`multiline.ts`) makes a `BATCH … draft/multiline <target>` one message: inbound, its lines are joined with `\n` (`draft/multiline-concat` appends without one) and carry the _opener's_ `time`/`msgid`/`account`/client tags but the _lines'_ source; outbound, `dispatchInput` groups each run of consecutive plain-text lines into one `sendMultiline`, and commands stay one per line. Anything that renders or truncates `msg.text` must cope with `\n`. The server drops a rejected batch whole and answers `FAIL BATCH …` (nefarious2 charges a per-batch cooldown), so every batch sent is remembered until answered and re-sent as one message per line if it FAILs.
