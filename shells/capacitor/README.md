...
- **The paperclip's file picker offers the camera** (Android) → `UploadChooserClient.java` wraps the system picker in a chooser with a photo and a video capture intent up front (Camera / Camcorder / Media picker / Files, the way iOS's sheet offers Take Photo or Video), filtered by the input's `accept` list. No `CAMERA` permission is declared — an app without it may still ask the camera app for a shot — the shot lands in a cache file the `FileProvider` exposes, and the manifest's `<queries>` makes the capture actions visible on Android 11+. An input with `capture` still goes Capacitor's way. Checked on the emulator: chooser, shot → upload preview, cancel.
