...
- **`App.backButton`** (Android) → whatever is open on top goes first: an open image, then any overlay that closes on Escape, then the phone's sidebar or user-list overlay; with nothing left to close, a standalone page gives way to its conversation, and then `App.minimizeApp()`. An overlay declares itself with `data-escape-close` on its root while it is up (`ContextMenu`, `Mentions`, `ReactionPicker`, `ConfirmDialog`, `PushPrompt`, `UploadPreview` — the same components that subscribe to the `escapekey` eventbus event while open), which is the one thing `native.ts` looks for; a new overlay that wants the back button has to carry it. Never `router.back()`: the history is kept one deep (`router.ts`).
